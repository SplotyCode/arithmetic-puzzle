...
public class MadMax implements HexBinary {
    ...
    private static Set<int[]> groups = new HashSet<>();
    private static long[] cards;

    private static void helper(int[] indexes, int start, int end, int index) {
        if (index == indexes.length) {
            ...
            indexes[index] = start;
            helper(indexes, start + 1, end, index + 1);
            helper(indexes, start + 1, end, index);
        }
    }

    private static boolean a(int[] indexes) {
        long value = cards[indexes[0]];
        for (int i = 1; i < indexes.length; i++) {
            value ^= cards[indexes[i]];
        }
        return value == 0;
    }
}
